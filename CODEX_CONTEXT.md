...
- 데스크톱 공통 Apps 사이드바와 모바일 헤더/하단 내비게이션이 구현됨.
- 2026-09-15 대시보드 개편: 224px 사이드바, 상단 현재 위치 표시, 넓은 공통 작업 영역과 작은 페이지 제목을 사용한다. `app/dashboard.css`를 `app/globals.css` 뒤에 로드하며 기능별 화면도 같은 카드·간격·반응형 규칙을 적용한다.
...
- 홈 대시보드는 Supabase 기록을 바탕으로 집중, 루틴, 수면, 운동 요약을 표시함.
- 홈은 요약 카드 4개, 최근 7일 실제 집중 시간(분), 오늘 할 일 최대 5개, 예정 일정 최대 4개, 집중 시작·오늘 루틴·바로가기 패널로 구성한다. 임의 가중 활동 점수는 제거했다. 루틴은 오늘 반복 요일에 해당하는 항목만 집계하고 수면 카드는 `/sleep`으로 연결한다. 로딩·조회 오류를 0건과 구분한다.
...
- 현재 PC에는 `.env.local`이 없다. 운영 인증·실제 데이터·DB 마이그레이션 적용 여부는 미검증이다. 로컬 브라우저 QA는 격리된 테스트 컨텍스트에서 GET 응답만 대체했으며 운영 데이터를 변경하지 않았다.
...
- 2026-09-15: 전체 대시보드 레이아웃과 홈 통합 요약 적용. production build, TypeScript 검사, 변경 컴포넌트 lint 통과. Edge headless에서 주요 9개 화면 × 1440/1024/390px(27조합) 가로 넘침·페이지 오류 없음. 홈 시각 확인, 사이드바 접기/새로고침 복원, 수면 카드 이동, 모바일 메뉴 이동, 빈 데이터·오류 상태 검증. 인증/기록 GET은 테스트 응답을 사용했으며 실제 운영 연동과 CRUD는 미검증. 전체 lint의 기존 진단 수는 재검증하지 않았다. Sites 프로젝트 접근 불가로 게시 미완료.
