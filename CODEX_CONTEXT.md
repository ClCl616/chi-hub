...
- **2026-09-16 학식 변경은 로컬 커밋만 완료했고 GitHub에 push하지 않았다.** 이번 문서 정리 시작 시 작업 트리는 깨끗했고 `main`은 로컬 추적 참조 `origin/main`(`a87c49d`)보다 1개 커밋 앞섰다. 이 문서 커밋도 전달 대상이다. 원격 서버는 재조회하지 않았으므로 실제 원격 최신 상태를 단정하지 않는다.
...
1. **이번 변경이 포함된 프로젝트 전체를 확보한다.** `235ee93`과 그 뒤의 인수인계 문서 커밋이 포함돼야 한다. 현재 GitHub clone/pull만으로 최신 변경을 받는다고 가정하지 말고 아래 전달 방법과 `git log`를 확인한다.
...
- GitHub 동기화 기록은 2026-09-15의 `a87c49d`까지다. 2026-09-16 `235ee93`과 이번 후속 문서는 아직 GitHub에 push하지 않았다. 이번 요청은 문서 정리이므로 origin push나 사이트 게시를 수행하지 않는다.
...
- 2026-09-16 · `235ee93`: 학식 화면에서 충북대학교·한국외국어대학교 선택 버튼 제거. 기존 학교 저장값 검증으로 대전대학교 기본값 유지, 서버 API 보존. Production build·로컬 학식 페이지 HTTP 200·diff 검사 통과. 로컬 커밋 완료, GitHub 미업로드. Sites 조회·소스 접근 요청은 `project_not_found`로 게시 미완료.
