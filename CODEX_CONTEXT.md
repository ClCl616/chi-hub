...
- CHI.HUB 전체가 하나의 대시보드다. 별도 대시보드 탭은 없으며 224px 사이드바와 모바일 메뉴는 같은 페이지의 작업 패널 앵커로 이동한다. `app/dashboard.css`를 `app/globals.css` 뒤에 로드한다.
- `/focus`, `/morning`, `/notes`, `/calendar`, `/sleep`, `/workouts`, `/files`, `/meals`는 각각 `/#기능명`으로 리다이렉트한다. 로그인 복귀 경로는 해시도 보존한다.
...
- `DashboardWorkspace`는 타이머·루틴/할 일·메모·캘린더·수면·운동·드라이브·학식의 실제 작업 컴포넌트를 8개 패널에 배치한다. 이전 요약 카드/차트와 개별 앱 이동 방식은 제거했다.
- `WorkspacePanel`은 패널 접기와 넓게 보기를 제공한다. 접기/확대/앵커 이동 때 컴포넌트를 언마운트하지 않아 타이머·메모·입력 폼 상태를 유지한다. 접힌 패널의 메뉴를 다시 누르면 펼치고 포커스를 이동한다. 일반 패널은 내부 스크롤, 모바일 확대는 전체 높이를 사용한다.
- 타이머 완료 창은 body 포털로 렌더링하므로 타이머 패널을 접은 상태에서도 표시된다.
