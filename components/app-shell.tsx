...
export function AppShell({children}:{children:React.ReactNode}){
 ...
 const [authState,setAuthState]=useState<'checking'|'ready'|'offline'>('checking');
 useEffect(()=>{fetch('/api/auth/status',{cache:'no-store'}).then(response=>{if(response.status===401){window.location.href=`/login?returnTo=${encodeURIComponent(pathname)}`;return}setAuthState(response.ok?'ready':'offline')}).catch(()=>setAuthState('offline'))},[pathname]);
 ...
 return <div className="app-shell">
  ...
  <main className="main-content">{authState==='checking'?<div className="auth-check" role="status">내 공간을 불러오는 중…</div>:<>{authState==='offline'&&<div className="service-banner" role="status">데이터 연결을 확인해주세요. 일부 기능이 제한될 수 있습니다.</div>}{children}</>}</main>
